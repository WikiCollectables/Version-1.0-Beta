...
<form id='deleteconfirm' method='post' action=\"{$formaction}\">
	...
	function doDeleteArticle( $reason ) {
		...
		# Now that it's safely backed up, delete it ### Added to for buy/sell feature extension ###
		# Bug # 66 - "Delete" article tab: The delete tab deletes the article. In this case, the corresponding ads and escrows should be deleted from the database 
		global $wgDBprefix;
		$sql="DELETE FROM escrow_history 
				WHERE escrow_id IN ( 
										SELECT e.id FROM " . $wgDBprefix. "ads a, escrow_escrow e 
															WHERE e.ad_id = a.ad_id and a.page_id = " . $id . " ) ";
		$dbw->query($sql);

		$sql="DELETE FROM escrow_reports 
				WHERE escrowid IN ( 
										SELECT e.id FROM " . $wgDBprefix. "ads a, escrow_escrow e 
															WHERE e.ad_id = a.ad_id and a.page_id = " . $id . " ) ";
		$dbw->query($sql);

		$sql="DELETE FROM escrow_shipping_status 
				WHERE escrow_id IN ( 
										SELECT e.id FROM " . $wgDBprefix. "ads a, escrow_escrow e 
															WHERE e.ad_id = a.ad_id and a.page_id = " . $id . " ) ";
		$dbw->query($sql);

		$sql="DELETE FROM escrow_shipping_v 
				WHERE escrow_id IN ( 
										SELECT e.id FROM " . $wgDBprefix. "ads a, escrow_escrow e 
															WHERE e.ad_id = a.ad_id and a.page_id = " . $id . " ) ";
		$dbw->query($sql);

		$sql="DELETE FROM escrow_usps_xml 
				WHERE escrow_id IN ( 
										SELECT e.id FROM " . $wgDBprefix. "ads a, escrow_escrow e 
															WHERE e.ad_id = a.ad_id and a.page_id = " . $id . " ) ";
		$dbw->query($sql);

		$sql="DELETE FROM escrow_escrow WHERE ad_id IN ( SELECT ad_id FROM " . $wgDBprefix. "ads WHERE page_id = " . $id . " ) ";
		$dbw->query($sql);

		$sql="DELETE FROM " . $wgDBprefix. "adimage WHERE ad_id IN ( SELECT ad_id FROM " . $wgDBprefix. "ads WHERE page_id = " . $id . " ) ";
		$dbw->query($sql);

		$dbw->delete( 'ads', array( 'page_id' => $id ), __METHOD__);
