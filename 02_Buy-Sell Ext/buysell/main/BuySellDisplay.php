...
class BuySell {
	...
	/**
	 * Gets the latest moved article name.
	 *
	 * @returns latest moved article name
	 */
	function getRedirectedTitle_v($page_id)
	{
		global $wgDBprefix;
		$dbr =& wfGetDB( DB_SLAVE );
		$sql = "SELECT pl_title from " . $wgDBprefix . "pagelinks where pl_from = '" . $page_id . "' ";
		...
		if($line->pl_title != "") 
		{
			...
			{
				$my_page = $this->getRedirectedTitle_v($line->page_id);
			}
		}
		return $my_page;
	} // getRedirectedTitle_v($page_id)
	 ...
	 */
	function buysellart() {
		global $wgOut, $wgRequest, $wgTitle, $wgDBprefix;
		...
		// index.php?title=BuySell&action=buysell&user_id=2

		$page_id = $_REQUEST[page_id];
		$user_id = $_REQUEST[user_id];

		# if ?title=Draped_Bust_half_cent&action=buysell&page_id=1557
		$sql = "SELECT page_id, page_title, page_is_redirect from " . $wgDBprefix . "page where page_id = '" . $page_id . "' ";
		$res = $dbr->query($sql, __METHOD__);
		$line = $dbr->fetchObject( $res ) ;
		if($line->page_id > 0) 
		{
			$my_page = str_replace("_", " ", $line->page_title);
			$my_page_id = $line->page_id;
			// if article is redirected
			if($line->page_is_redirect == 1) 
			{
				$my_page = $this->getRedirectedTitle_v($line->page_id);
				$my_page_id = $this->getRedirectedID_v($line->page_id);
				
				/*$sql = "SELECT pl_title from pagelinks where pl_from = '" . $line->page_id . "' ";
				$res = $dbr->query($sql, __METHOD__);
				$line = $dbr->fetchObject( $res ) ;
				if($line->pl_title != "") 
				{
		...
		# if Special:BuySell/username
		$arr = explode(":", $arr[count($arr) - 1]);
		$sql = "SELECT user_id, user_name from " . $wgDBprefix . "user where user_id = '" . $user_id . "' ";
		$res = $dbr->query($sql, __METHOD__);
		$line = $dbr->fetchObject( $res ) ;
		if($line->user_name != "")
		{
			$my_user_name = str_replace("_", " ", $line->user_name);
		}
		$wgOut->setPageTitle( 'Buy/Sell: ' . $my_page.$my_user_name );

		# first check for user_id. if particular user's buy/sell page required
		if($user_id > 0)
			$my_wikiUserID=$user_id;
		// else take session wsUserID
		else
			$my_wikiUserID=$_SESSION[wsUserID];

		$my_wikiPageID=$my_page_id;

		include("extensions/buysell/main/buysell.php");
		$wgOut->addHTML($buysell_content);
...
Comment out "There is no history for this page" message on user buy/sell*/
